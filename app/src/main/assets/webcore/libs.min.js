...
$.when(
    ...
    $.Deferred(function() {
        var that = this;
        $(document).on('i18n', function() {
            that.resolve();
        });
    })
).done(function() {
    $(document).trigger('drawarrow');
});

$(document).on('drawarrow', function() {
    // right arrow for listview
    $('[data-role="listview"] > li > a').not('.delete-button').not('.add-button').each(function() {
        // make sure just append once
        if (!$(this).find('.arrow-right').length) {
            $(this).append('<img class="arrow-right" src="../webcore/images/arrow-right.png" />');
        }
    });

    // delete button
    $('[data-role="listview"] > li > a.delete-button').each(function() {
        // make sure just append once
        if (!$(this).find('.delete-icon').length) {
            $(this).append('<img class="delete-icon" src="../webcore/images/close.png" />');
        }
    });

    // left arrow for back button
    $('[data-icon="arrow-l"]').each(function() {
        if (!$(this).find('.arrow-left').length) {
            $(this).prepend('<img class="arrow-left" src="../webcore/images/arrow-left.png" />');
        }
    });

    // add button
    $('[data-role="listview"] > li > a.add-button').each(function() {
        // make sure just append once
        if (!$(this).find('.add-icon').length) {
            $(this).append('<img class="add-icon" src="../webcore/images/add.png" />');
        }
    });

    // add input clear button
    $('[data-role="listview"] > li > input[type="text"],  \
       [data-role="listview"] > li > input[type="number"],\
       .show-hidden-block > input').each(function(){
        if ($(this).siblings('.clear-btn').length > 0 ||
            $(this).attr('data-clear') === "false") {
            return true;
        }

        var that = this,
        img = document.createElement('img');
        img.className = 'clear-btn';
        img.src = '../webcore/images/clear.png';
        img.onclick = function(){$(that).val('');};
        this.parentNode.appendChild(img);

        // compute distance to the right
        function setOffsetRight()
        {
            var left = that.offsetLeft + that.offsetWidth;
            var right = ((left === 0) ? 8 : (document.body.offsetWidth - left + 8)) + 'px';

            $(img).css('right', right);
        }

        setOffsetRight();
        $(document).on('pagechange', setOffsetRight);
        ...
        function callback(event)
        {
            ...
            if (event.type === 'blur' || this.value === '')
                setTimeout(function(){$(img).hide();}, 0);
            else
                setTimeout(function(){$(img).show();}, 0);
        }
